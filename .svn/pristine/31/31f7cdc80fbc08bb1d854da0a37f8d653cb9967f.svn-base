...
@implementation TMStatusUtility
...
{
    if (YES) {
        return 1004.0 - [TMStatusUtility getNavigationBarHeight] - [TMStatusUtility getTabBarHeight] - 51;
    }else{
        return 460.0 - [TMStatusUtility getNavigationBarHeight] - [TMStatusUtility getTabBarHeight];
    }
//return 0.0;
}
 ...
 *	@param 	l_data 	详情对象
 */
+(void)saveBrowerData:(TMBaseData*)l_data{
 ...
 *	@param 	l_data 	浏览记录对象
 */
+(void)deleteBrowerData:(TMBaseData*)l_data{
...
+(void)clearAllMermoryCache{
    int l_int_sizeM=[[SDImageCache sharedImageCache] getSize]/1024/1024;
...
//    DLog(@"磁盘%d",[[SDImageCache sharedImageCache] getMemorySize]);
//    DLog(@"内存%d",[[SDImageCache sharedImageCache] getSize]);
    if ([TMStatusUtility isAllImgCacheClean]) {
        [TMStatusUtility showAlert:nil message:@"缓存清理成功！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
    }else{
        [TMStatusUtility showAlert:nil message:@"缓存清理失败！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
    }
}
...
+(BOOL)isAllImgCacheClean{
    ...
    if ([[SDImageCache sharedImageCache] getDiskCount]==0&&[[SDImageCache sharedImageCache] getSize]==0) {
        return YES;
    }else{
        return NO;
    }
